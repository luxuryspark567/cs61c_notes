...
* Positive Max: <mark style="color:orange;">1</mark> \* <mark style="color:yellow;">(1 - 2^(-23))</mark> \* <mark style="color:blue;">2^(-126)</mark> = 2^(-126) - 2^(-149) = 1.175\*10^(-38)
...
{% hint style="info" %}
(1) The smallest <mark style="color:red;">exponent (−126) ensures a smooth transition</mark> between the smallest normalized value ( 1.0 × 2 ^(−126) ) and the largest denormalized value. Denormalized numbers are used to represent values closer to zero, maintaining gradual underflow.
{% endhint %}
...
* Allocate bits into the appropriate fields.
  * Sign: 0, Exp: (8 - bias), <mark style="color:yellow;">Mantissa: 01101101101</mark>
  * <mark style="color:red;">If we have a resulting Exp = order + bias = 0, and a leading 1 in our number (i.e. 1.1010... \* 2^(order)), then we have a denorm float (Exp = order - (bias + 1))</mark>
