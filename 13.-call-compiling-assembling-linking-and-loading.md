...
* Interpreter: <mark style="color:red;">Directly executes</mark> a program in the source language
...
Some languages, like <mark style="color:blue;">Java</mark>, use a hybrid approach, compiling code into <mark style="color:red;">intermediate bytecode</mark> <mark style="color:red;">executed by an interpreter (JVM)</mark>.
{% endhint %}
...
* Assembler (.s -> .o)
  ...
  * Output: Object File and Information for linking and debugging&#x20;
    ...
    * Make two passes over the code
      * First:
        * Resolves pseudo-instructions
        * Build Symbol Table:
          * Labels and their Addresses
          * Anything in .data
          * Variables used across files(.global)
      * Second:
        * Resolves forward references
    * Builds Relocation Table:
      * Stores forward reference labels (label use before declaration)
...
*   Linker (.o -> .out)
    ...
    * Output: Executable code
      * Combines text and data segments of different .o files in order (determined by linker)
      * Resolves references (fills in all absolute addresses in relocation table), .
